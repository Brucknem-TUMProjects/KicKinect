...
//	public bool NearMode = false;
    ...
    public virtual Texture2D GetUsersLblTex()
    { 
		return usersLblTex;
	}
	...
	public virtual bool IsUserDetected()
	{
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								if(HandCursor2 != null)
								{
									...
									if(HandCursor2.GetComponent<Text>() == null)
									{
								...
								if(ControlMouseCursor)
								{
									Vector3 vCursorPos = HandCursor2.GetComponent<Text>() != null ? HandCursor2.transform.position :
